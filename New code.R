...
dat.pathogen=dat[grep("NC",dat$Sequence.name,fixed = TRUE),]
dat.pathogen.wm<-dat.pathogen[,-c(1:9,11:28,47:61)]
Attributes <-dat.pathogen.wm[,c(1)]
Attributes<-str_split_fixed(Attributes,":", n=Inf)
Attributes<-Attributes[,c(2)]
Attributes<-str_split_fixed(Attributes,",", n=Inf)
Attributes<-Attributes[,-c(2)]
dat.pathogen.wm<-cbind(dat.pathogen.wm[,1],Attributes,dat.pathogen.wm[,2:19])
dat.pathogen.wm<-dat.pathogen.wm[,-c(1)]
dat.pathogen.wmp=dat.pathogen.wm[grep("E1W",dat.pathogen.wm$Attributes,fixed = TRUE),]

#mean of same uniprotID
raw.agg<-aggregate(.~ Attributes,dat.pathogen.wmp, FUN="mean")
#making rownames 1,2,3 as uniprotID name
rownames(raw.agg) <- raw.agg[,1]
raw.agg <- raw.agg[,-1]
dat.pathogen.uniq<-raw.agg
dt=dat.pathogen.uniq[rowSums(is.na(dat.pathogen.uniq[,-1])) !=ncol(dat.pathogen.uniq[,-1]),]
...
out.Wt2h <- topTable(fit.cont, n = Inf, coef = 1)
# add zero counts to each table
out.Wt4h <- topTable(fit.cont, n = Inf, coef = 2)
# repeat for the rest
out.Wt8h <- topTable(fit.cont, n = Inf, coef = 3)
out.Wt16h <- topTable(fit.cont, n = Inf, coef = 4)
out.Wt24h <- topTable(fit.cont, n = Inf, coef = 5)
...
write.csv(y.norm, file = "y.norm.csv")



out.Wt2h <- tibble::rownames_to_column(out.Wt2h, "Gene Name")
out.Wt4h <- tibble::rownames_to_column(out.Wt4h, "Gene Name")
out.Wt8h <- tibble::rownames_to_column(out.Wt8h, "Gene Name")
out.Wt16h <- tibble::rownames_to_column(out.Wt16h, "Gene Name")
out.Wt24h <- tibble::rownames_to_column(out.Wt24h, "Gene Name")

write.csv(out.Wt2h, file = "out.wt2h.csv", row.names = FALSE)
write.csv(out.Wt4h, file = "out.wt4h.csv", row.names = FALSE)
write.csv(out.Wt8h, file = "out.wt8h.csv", row.names = FALSE)
write.csv(out.Wt16h, file = "out.wt16h.csv", row.names = FALSE)
write.csv(out.Wt24h, file = "out.wt24h.csv", row.names = FALSE)
...
#filter <- apply(dat.pathogen.na, 1, function(x){length(x[x>10])>=3}) (based on the paper information # select rows with expression at least 10 in at least 3 replicates)
...
temp = setdiff(as.character(alldt2h.up$Attributes2h), as.character(alldt4h.up$Attributes4h))
